...
class PesertaSeleksi extends Page implements Tables\Contracts\HasTable
{
    ...
    //membuat action masal pada data record, yaitu hapus data
    ...
    //membuat tombol action pada baris, yaitu untuk ubah status hasil seleksi
    ...
    //membuat fungsi simpan, untuk menyimpan data peserta baru
    ...
    //menambahkan tombol action tambah diatas table
                    ...
                    Forms\Components\Select::make('no_daftar')
                        ...
                        ->getSearchResultsUsing(function ($search) {
                            return ViewFormulirPeserta::select(
                                "no_daftar",
                                DB::raw("CONCAT(no_daftar,' - ',nama) as 
                                 label")
                            )->where(
                                    'nama',
                                    'like',
                                    "%" . $search . "%"
                                )->whereNull("id_seleksi")->limit(5)
                                ->pluck('label', 'no_daftar');
                        })
                        ->required(),
                ])
